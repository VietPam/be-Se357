...
class addressController {
  async addAddress(req, res) {
    const { userId, nameAddress, city, district, street, detailLocation } =
      req.body;
    if (
      !userId ||
      !nameAddress ||
    ...
    try {
      ...
      res
        .status(200)
        .json({ code: 0, message: "Add new Address successfully", data });
    } catch (e) {
      res.status(500).json({ errCode: 1, errMessage: "Internal server error" });
    }
  }
  async updateAddress(req, res) {
    const { addressId, nameAddress, city, district, street, detailLocation } =
      req.body;
    ...
    try {
      ...
      if (!address) {
        return res
          .status(404)
          .json({ errCode: 404, errMessage: "Address not found" });
      }
  ...
  async getAllAddressByUserId(req, res) {
    ...
    try {
      ...
      res
        ...
        .json({
          code: 0,
          message: "Get addresses by userId successful.",
          data: address,
        });
    } catch (error) {
      console.error(error);
  ...
  async deleteAddress(req, res) {
    ...
    try {
      ...
      res
        .status(200)
        .json({ code: 0, message: "Deleted address successfully", data });
    } catch (error) {
      console.error(error);
      res
        .status(500)
        .json({ errCode: 500, errMessage: "Internal server error" });
    }
  }
}
